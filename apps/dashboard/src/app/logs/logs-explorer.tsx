    ...
    <div className="flex max-w-7xl flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className={
                appliedFilters.eventName === tab.eventName
                  ? "rounded-md border border-indigo-500/40 bg-indigo-500/10 px-3 py-1 text-xs font-medium text-indigo-400"
                  : "rounded-md border border-slate-700 px-3 py-1 text-xs font-medium text-slate-400 hover:border-slate-600 hover:text-slate-200"
              }
            >
              {tab.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-48 rounded-md border border-slate-700 bg-slate-800/50 px-3 py-1.5 text-sm text-slate-200 placeholder:text-slate-600 focus:border-slate-500 focus:outline-none"
          />
          ...
          <div className="flex items-center gap-1.5">
            {realtimeStatus === "live" ? (
              <>
                ...
                <span className="text-xs text-indigo-400">ライブ</span>
              </>
            ) : (
              <>
                <span className="h-2 w-2 rounded-[50%] bg-slate-600" />
                <span className="text-xs text-slate-500">{realtimeMeta.label}</span>
              </>
            )}
          </div>
        </div>
      ...
      {nextCursor && (
        <div className="flex justify-center">
          <button
            className="rounded-md border border-slate-700 px-4 py-2 text-sm text-slate-400 hover:border-slate-600 hover:text-slate-200 disabled:opacity-40"
    ...
    <li>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-0.5 text-xs text-slate-600">{log.eventName}</p>
        </div>
      ...
      {isExpanded && (
        <div className="border-t border-slate-800/60 bg-slate-950/50 px-4 py-3">
          <dl className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs sm:grid-cols-3">
            <div>
              <dt className="text-slate-600">イベント時刻</dt>
            ...
            {log.actorId && (() => {
              ...
              return (
                <div>
                  <dt className="text-slate-600">アクター</dt>
                  <dd className="text-slate-300">
                    {name ? (
                      <>
                        <span>{name}</span>
                        <span className="ml-1 font-mono text-xs text-slate-500">({log.actorId})</span>
                      </>
                    ) : (
            ...
            {log.channelId && (() => {
              ...
              return (
                <div>
                  <dt className="text-slate-600">チャンネル</dt>
                  <dd className="text-slate-300">
                    {name ? (
                      <>
                        <span>#{name}</span>
                        <span className="ml-1 font-mono text-xs text-slate-500">({log.channelId})</span>
                      </>
                    ) : (
            ...
            {Object.entries(payload)
              ...
              .map(([k, v]) => (
                <div key={k}>
                  <dt className="text-slate-600">{payloadFieldLabel(k)}</dt>
                  <dd className="truncate text-slate-300">{formatPayloadFieldValue(k, v)}</dd>
                </div>
              ))}
          </dl>
          ...
          {canViewRaw && (
            <details className="mt-3">
              ...
              <pre className="mt-1 max-h-64 overflow-auto rounded bg-slate-900 p-2 text-xs text-slate-400">
