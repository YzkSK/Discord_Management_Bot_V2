...
## Section 5: ダッシュボードUI文言

**ファイル:** `apps/dashboard/src/lib/locale.ts`（新規作成）

Bot localeと同じ型付きオブジェクト方式。ダッシュボード専用の `DashboardLocale` 型を定義する。

### 言語決定ロジック

```typescript
// Guild未読み込み時: ブラウザ言語から判定
function detectBrowserLanguage(): GuildLanguage {
  const lang = navigator.language; // 例: "ja", "ja-JP", "en-US"
  return lang.startsWith("ja") ? "ja" : "en";
}

// Guild読み込み後: Guild設定の language を優先する
```

### 対象文言（settings-panel.tsx / logs-explorer.tsx / その他ページ）
...
- `apps/dashboard/src/lib/locale.ts` に `getDashboardLocale(lang: GuildLanguage): DashboardLocale` を定義
- `settings-panel.tsx` に `const [uiLang, setUiLang] = useState<GuildLanguage>(() => detectBrowserLanguage())` を追加
- Guild読み込み成功後: `setUiLang(data.language as GuildLanguage)`
- `const loc = getDashboardLocale(uiLang)` でUI文言を取得し、JSX内の文字列を置き換える

---

## Section 6: スラッシュコマンドのDescription多言語化

Discordのネイティブlocalization APIを使用する。コマンドピッカーに表示される `name`・`description`・各オプションのdescriptionをDiscord側で自動切り替えする。

**対象ロケールコード:**
- 英語: `en-US`
- 日本語: `ja`（Discordが対応するロケールコード）

**変更ファイル:** `apps/bot/src/commands/setup.ts`・`recruitment.ts`・`tts.ts`
...
全コマンド・サブコマンド・オプションに日本語localizationを追加する。変更後は `register-commands.ts` を実行してDiscordにコマンドを再登録する必要がある。
...
| 状況 | フォールバック |
|------|--------------|
| guildIdなし（DM等） | Bot: `'en'` |
| GuildがDBに未登録 | Bot: `'en'` |
| DB取得エラー | Bot: `'en'` |
| DB内に不明な言語値 | `'en'`（`isGuildLanguage` ガードで検出） |
| Guild未読み込み（ダッシュボード） | `navigator.language` でen/ja判定 |
| スラッシュコマンドdescription | Discord側でユーザーのクライアント言語に応じて自動選択 |
