...
- Add `guildLanguages = ['en', 'ja'] as const` and `GuildLanguage` type
- Add `isGuildLanguage(value: string): value is GuildLanguage` type guard
- `getGuildConfigByGuildId`: include `language` in SELECT
- `updateGuildConfigByGuildId`: add optional `language?: GuildLanguage` to input; include in upsert SET when provided
...
Export `guildLanguages`, `GuildLanguage`, `getLocale` from `packages/shared/src/index.ts`.
...
- Validate with `isGuildLanguage(language)`; return 400 if invalid
